...
void parallel_merge_sort(int * array, unsigned int left, unsigned int right, unsigned int depth=0) {
	...
	if(depth >= num_workers){
		sequential_merge_sort(array, left, right); // if the depth of the merge sort is equal to or less than the total amount of available threads
													// then perform normal sequential merge sort
	}
	else //else spawn a left thread to perform the same arithmetic on the left side and use the current thread to perform arithmetic on the
	{
		unsigned int mid = (left + right) /2;
		std::thread left_thread = std::thread(parallel_merge_sort, array, left, mid, depth+1); // and increase the depth
